...
void apply_move_to_tree(TreeNode **root, int move, int initial_tree_depth) {
    if (!root || !(*root) || move < 0 || move >= (*root)->num_children) {
        return;
    }

    TreeNode *selected_child = (*root)->children[move];

    // Reset the children array except for the selected move.
    ...
    // Free the root node itself (but not the selected child)
    free((*root)->children);
    free_game_state((*root)->game_state);
    free(*root);

    // Update root to be the selected child
    *root = selected_child;

    // First expansion: Always expand one more depth
    expand_tree(*root);

    // Calculate the minimum expected node count
    int min_nodes = 1;
    for (int i = 0; i < initial_tree_depth - 2; i++) {
        min_nodes *= (*root)->game_state->width;
    }

    // Second expansion: If node count is less than game_width^(depth-2)
    if (node_count(*root) < min_nodes) {
        expand_tree(*root);
    }
}
