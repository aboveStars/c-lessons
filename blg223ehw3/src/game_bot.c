...
int get_max(TreeNode* node) {
    ...
    // If it is leaf node, we directly evaluate score and returns it.
    ...
    for(int i =0; i < node->num_children; i++) {
        int val = get_min(node->children[i]);
        if(val > max_value) {
            max_value = val;
        }
    }
...
int get_min(TreeNode* node) {
    ...
    for (int i=0; i<node->num_children; i++) {
        int val = get_max(node->children[i]);
        if(val < min_value) {
            min_value = val;
        }
    }
...
void eval_game_tree(TreeNode *root) {
    ...
    // If it is a leaf node, return game state.
...
int best_move(TreeNode* root) {
    if(root == NULL || root->num_children == -1) {
        return -1;
    }
    ...
    for (int i = 0; i <root->num_children; i++) {
        int move_val = get_min(root->children[i]);
